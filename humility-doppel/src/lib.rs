...
pub enum CounterVariant {
    ...
    Single(u32),
    /// A nested set of counters, as generated by a variant annotated with
    /// `#[count(children)]`.
    Nested(Counters),
}
...
pub mod hiffy {
    ...
    pub const OP_WRITE_TEXT: u16 = 1;
    pub const OP_WRITE_DATA: u16 = 2;
    pub const OP_KICK: u16 = 3;
}
...
impl humility::reflect::Load for CountedRingbuf {
    fn from_value(v: &Value) -> Result<Self> {
        ...
        let ringbuf = rb_struct
            ...
            .transpose()?;
        let counters = Counters::from_value(&rb_struct["counters"])?;

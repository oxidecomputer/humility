...
struct RencmArgs {
    ...
    #[structopt(
        long, short, value_name = "register", conflicts_with = "module"
    )]
...
fn rencm(
    ...
    let modules = idt8a3xxxx::modules();
    ...
    if let Some(registers) = &subargs.register {
        for register in registers {
            let s = register.split(".").collect::<Vec<&str>>();
    ...
    info!("{} registers to read", work.len());
            ...
            match result {
                Ok(ref r) => {
                    ...
                    let payload = match idt8a3xxxx::Payload::from_slice(
                        ...
                        None => {
                            bail!("short read for {:?} at {:?}", job, result)
                        }
                    };

                    let value = payload.value();

                    let name = if job.0.base.len() == 1 {

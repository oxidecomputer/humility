...
) -> Result<Vec<((&'a str, &'a HubrisVariable), Option<&'a HubrisStruct>)>> {
...
pub fn ringbuf(    subargs: &RingbufArgs,
    hubris: &HubrisArchive,
    core: &mut dyn Core) -> Result<()> {

    //let subargs = RingbufArgs::try_parse_from(&context.cli.cmd)?;
    //let hubris = &context.cli.archive()?;

    let ringbufs = gen_ringbufs(hubris, &subargs.name)?; 

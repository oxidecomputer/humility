...
impl<'a> HiffyDashboardReader<'a> {
    fn new(
        ...
        Ok(Self { context, ops, status_op, num_status_items: 1 })
    }
...
impl RamDashboardReader {
    fn new(
        ...
        for temp_name in temps {
            if let Some((i, _)) =
                hubris.manifest.sensors.iter().enumerate().find(|(_, s)| {
                    &s.name == temp_name
                        && s.kind == HubrisSensorKind::Temperature
                })
            {
        ...
        for fan_name in fans {
            if let Some((i, _)) =
                hubris.manifest.sensors.iter().enumerate().find(|(_, s)| {
                    &s.name == fan_name && s.kind == HubrisSensorKind::Speed
                })
            {
                sensor_indices.push(i);
            }
...
impl DashboardReader for RamDashboardReader {
    fn read(
        ...
        Ok(DashboardData { power_state: None, sensor_values })
    }
...
impl<'a> Dashboard<'a> {
    fn new(
        ...
        for s in &hubris.manifest.sensors {
            match s.kind {
                ...
                HubrisSensorKind::Current => current.push(s.name.clone()),
                _ => {}
            }
        }
        ...
        let reader: Box<dyn DashboardReader> = match subargs.backend {
            ...
            Some(DashboardBackend::Readmem) => Box::new(
                RamDashboardReader::new(hubris, &temps, &fans, &current)?,
            ),
            None => {
                // Try hiffy first, fall back to readmem
                match HiffyContext::new(hubris, core, subargs.timeout) {
                    Ok(context) => {
                        match HiffyDashboardReader::new(hubris, context) {
                            Ok(r) => Box::new(r),
                            Err(_) => Box::new(RamDashboardReader::new(
                                hubris, &temps, &fans, &current,
                            )?),
                        }
                    }
                    Err(_) => Box::new(RamDashboardReader::new(
    ...
    fn need_update(&mut self, core: &mut dyn Core) -> Result<bool> {
        if self.last.elapsed().as_millis() > self.interval.into() {
            ...
            self.status =
                data.power_state.unwrap_or_else(|| "unknown".to_string());

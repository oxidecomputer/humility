...
mod inner {
    ...
    impl<'a> UartConsoleHandler<'a> {
        ...
        fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
            ...
            .context(
                "Could not find `ControlPlaneAgent.uart_read` operation",
            )?;
            ...
            let v = v
                .as_base()?
                .as_u32()
                .ok_or_else(|| anyhow!("Couldn't get U32"))?;
        ...
        fn write(&mut self, buf: &[u8]) -> Result<usize> {
            ...
            .context(
                "Could not find `ControlPlaneAgent.uart_write` operation",
            )?;
    ...
    fn stdout_writer(
        ...
        for data in rx {
            if let Some(log) = log.as_mut() {
                if let Err(err) =
                    log.write_all(&data).and_then(|()| log.flush())
                {
